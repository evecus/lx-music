...
import {
  importUserApiGroup,
  removeUserApiGroupWithSources,
  getGroupList,
} from '@main/modules/userApi/group'
...
export default () => {
  ...
  mainHandle<string, LX.UserApi.ImportUserApiGroup>(WIN_MAIN_RENDERER_EVENT_NAME.import_user_api_group, async({ params: url }) => {
    return importUserApiGroup(url)
  })
  mainHandle<string, LX.UserApi.UserApiInfo[]>(WIN_MAIN_RENDERER_EVENT_NAME.remove_user_api_group, async({ params: groupId }) => {
    return removeUserApiGroupWithSources(groupId)
  })
  mainHandle<LX.UserApi.UserApiGroupInfo[]>(WIN_MAIN_RENDERER_EVENT_NAME.get_user_api_group_list, async() => {
    return getGroupList()
  })
}
...
export const sendUserApiGroupUpdated = (info: { name: string, version: string }) => {
  sendEvent(WIN_MAIN_RENDERER_EVENT_NAME.user_api_group_updated, info)
}
