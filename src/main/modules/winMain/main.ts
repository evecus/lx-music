...
/**
 * 下载 MV 文件到指定保存路径
 * 使用 Electron 原生下载能力（session.downloadURL），支持大文件、断点续传（由浏览器内核处理）
 * onProgress 会在下载开始、进行中、完成/取消/出错时被调用
 */
export const downloadMvFile = async({ id, url, savePath, headers }: {
  id: string
  url: string
  savePath: string
  headers?: Record<string, string>
}, onProgress: (info: DownloadMvProgress) => void): Promise<void> => {
  if (!browserWindow) throw new Error('main window is undefined')
  const win = browserWindow
  const ses = win.webContents.session

  return new Promise<void>((resolve, reject) => {
    const listener = (_event: Electron.Event, item: Electron.DownloadItem, _webContents: Electron.WebContents) => {
      ...
      item.once('done', (_event, state) => {
        if (state === 'completed') {
          ...
          onProgress({
            id,
            status: state === 'cancelled' ? 'cancelled' : 'interrupted',
            receivedBytes: item.getReceivedBytes(),
            totalBytes: item.getTotalBytes(),
            errorMsg: state,
          })
          reject(new Error(state === 'cancelled' ? 'cancelled' : `download failed: ${state}`))
        }
      })
    }
    ses.on('will-download', listener)

    ses.downloadURL(url, headers ? { headers } : undefined)
  })
}
