...
export const importApi = async(scriptRaw: string, groupId?: string): Promise<LX.UserApi.UserApiInfo> => {
...
// ------------------------------------------------------------------
let userApiGroups: LX.UserApi.UserApiGroupInfo[] | null = null

export const getUserApiGroups = (): LX.UserApi.UserApiGroupInfo[] => {
  if (userApiGroups) return userApiGroups
  userApiGroups = (getStore(STORE_NAMES.USER_API_GROUP).get('userApiGroups') as LX.UserApi.UserApiGroupInfo[] | undefined) ?? []
  return userApiGroups
}

const saveGroupData = () => {
  getStore(STORE_NAMES.USER_API_GROUP).set('userApiGroups', userApiGroups ?? [])
}

export const addUserApiGroup = (info: LX.UserApi.UserApiGroupInfo) => {
  userApiGroups ??= []
...
export const updateUserApiGroup = (id: string, patch: Partial<LX.UserApi.UserApiGroupInfo>) => {
  userApiGroups ??= []
...
export const removeUserApiGroup = (id: string) => {
  userApiGroups ??= []
...
export const getUserApiIdsByGroup = (groupId: string): string[] => {
  return getUserApis().filter(api => api.groupId === groupId).map(api => api.id)
}
