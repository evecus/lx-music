...
export class AppEvent extends Event {
  ...
  download_task_add(task: LX.Download.DownloadTask) {
    this.emit('download_task_add', task)
  }

  download_progress_update(payload: { id: string, progress: LX.Download.DownloadTask['progress'] }) {
    this.emit('download_progress_update', payload)
  }

  download_status_update(payload: { id: string, status: LX.Download.DownloadTask['status'], errorMsg?: string }) {
    this.emit('download_status_update', payload)
  }

  download_metadata_update(payload: { id: string, metadataStatus: LX.Download.DownloadTask['metadataStatus'] }) {
