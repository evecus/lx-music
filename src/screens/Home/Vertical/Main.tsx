...
const DownloadPage = () => {
  const [visible, setVisible] = useState(commonState.navActiveId == 'nav_download_manager')
  const component = useMemo(() => <Download />, [])
  useEffect(() => {
    let currentId: CommonState['navActiveId'] = commonState.navActiveId
    const handleNavIdUpdate = (id: CommonState['navActiveId']) => {
      currentId = id
      if (id == 'nav_download_manager') {
        requestAnimationFrame(() => {
          setVisible(true)
        })
      }
...
const Main = () => {
  ...
  const component = useMemo(() => (
    ...
    >
      ...
      <View collapsable={false} key="nav_download_manager" style={styles.pageStyle}>
        <DownloadPage />
      </View>
