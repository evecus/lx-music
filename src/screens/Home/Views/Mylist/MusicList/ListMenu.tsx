...
export default forwardRef<ListMenuType, ListMenuProps>((props, ref) => {
  ...
  const handleSetMenu = (musicInfo: LX.Music.MusicInfo) => {
    ...
    const isOnlineMvSource = musicInfo.source == 'wy' || musicInfo.source == 'kg'
    const mvId = isOnlineMvSource ? (musicInfo.meta as LX.Music.MusicInfoMeta_online).mv ?? null : null
    const menu = [
    ...
    if (playMV && mvId) {
      menu.splice(3, 0, { action: 'playMv', label: t('play_MV') })
      menu.splice(4, 0, { action: 'downloadMv', label: t('download_MV') })
    }
    setMenus(menu)
