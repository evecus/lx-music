...
import { getMvPlayUrl, downloadMv } from '@/core/downloadMv'
...
// 播放MV：获取 MV 播放地址后弹出视频播放窗口
export const handlePlayMv = async(musicInfo: LX.Music.MusicInfoOnline) => {
  try {
    const url = await getMvPlayUrl(musicInfo)
    global.app_event.showVideoPlayer(url)
  } catch (err: any) {
    toast(err.message || global.i18n.t('mv_url_failed_tip'), 'long')
  }
}

// 下载MV到本地（默认系统 Movies 目录，可在设置中修改保存路径）
export const handleDownloadMv = (musicInfo: LX.Music.MusicInfoOnline) => {
  void downloadMv(musicInfo)
}
