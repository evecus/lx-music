...
dd(:aria-label="$t('setting__mv_path_title')")
  h3#download_mv_path {{ $t('setting__mv_path') }}
  div
    .p
      | {{ $t('setting__mv_path_label') }}
      span.auto-hidden.hover(:class="$style.savePath" :aria-label="$t('setting__mv_path_open_label')" @click="openDirInExplorer(appSetting['mv.savePath'])") {{ appSetting['mv.savePath'] }}
    .p
      base-btn.btn(min @click="handleChangeMvSavePath") {{ $t('setting__mv_path_change_btn') }}
...
export default {
  ...
  setup() {
    ...
    const handleChangeMvSavePath = () => {
      void showSelectDialog({
        title: t('setting__mv_select_save_path'),
        defaultPath: appSetting['mv.savePath'],
        properties: ['openDirectory'],
      }).then(result => {
        if (result.canceled) return
        updateSetting({ 'mv.savePath': result.filePaths[0] })
      })
    }
