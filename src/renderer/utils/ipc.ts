...
import { HOTKEY_RENDERER_EVENT_NAME, WIN_MAIN_RENDERER_EVENT_NAME, CMMON_EVENT_NAME, WIN_MV_RENDERER_EVENT_NAME } from '@common/ipcNames'
...
export const importUserApiGroup = async(url: string) => {
  return rendererInvoke<string, LX.UserApi.ImportUserApiGroup>(WIN_MAIN_RENDERER_EVENT_NAME.import_user_api_group, url)
}
export const removeUserApiGroup = async(groupId: string) => {
  return rendererInvoke<string, LX.UserApi.UserApiInfo[]>(WIN_MAIN_RENDERER_EVENT_NAME.remove_user_api_group, groupId)
}
export const getUserApiGroupList = async() => {
  return rendererInvoke<LX.UserApi.UserApiGroupInfo[]>(WIN_MAIN_RENDERER_EVENT_NAME.get_user_api_group_list)
}
export const onUserApiGroupUpdated = (listener: LX.IpcRendererEventListenerParams<{ name: string, version: string }>): RemoveListener => {
  rendererOn(WIN_MAIN_RENDERER_EVENT_NAME.user_api_group_updated, listener)
  return () => {
    rendererOff(WIN_MAIN_RENDERER_EVENT_NAME.user_api_group_updated, listener)
  }
}

export const openMvPlayer = (info: { url: string, name: string, singer: string }) => {
  rendererSend(WIN_MV_RENDERER_EVENT_NAME.open, info)
}

export interface DownloadMvProgress {
  id: string
...
/**
 * 下载 MV 到本地
 * 会先弹出保存对话框，用户确认后再开始真正下载；用户取消保存对话框时 canceled 为 true
 */
export const downloadMv = async(info: {
  id: string
  url: string
  savePath: string
  fileName: string
  headers?: Record<string, string>
}): Promise<{ canceled: boolean, savePath?: string }> => {
  return rendererInvoke<typeof info, { canceled: boolean, savePath?: string }>(WIN_MAIN_RENDERER_EVENT_NAME.download_mv, info)
}

/**
 * 监听 MV 下载进度
 */
export const onDownloadMvProgress = (listener: LX.IpcRendererEventListenerParams<DownloadMvProgress>): RemoveListener => {
  rendererOn(WIN_MAIN_RENDERER_EVENT_NAME.download_mv_progress, listener)
  return () => {
    rendererOff(WIN_MAIN_RENDERER_EVENT_NAME.download_mv_progress, listener)
  }
}
