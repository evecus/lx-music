...
/**
 * 获取在线 MV 播放地址
 * wy（网易云）直接请求官方公开接口解析，不依赖自定义源脚本；
 * kg（酷狗）暂无已验证可用的解析方式，调用时会直接抛错。
 * 没有"换源重试"的概念——若当前源不支持/解析失败，直接把错误抛给调用方。
 */
export const handleGetOnlineMvUrl = async({ musicInfo }: {
  musicInfo: LX.Music.MusicInfoOnline
}): Promise<{
  url: string
}> => {
  if (musicInfo.source != 'wy' && musicInfo.source != 'kg') throw new Error('该源不支持播放MV')
  if (!musicInfo.meta.mv) throw new Error('该歌曲没有MV')

  let reqPromise
  try {
    reqPromise = musicSdk[musicInfo.source].getMvUrl(toOldMusicInfo(musicInfo)).promise
  } catch (err: any) {
    reqPromise = Promise.reject(err)
  }
  return reqPromise.then(({ url }: { url: string }) => {
