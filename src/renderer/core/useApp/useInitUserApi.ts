...
import { onUserApiStatus, getUserApiList, getUserApiGroupList, onUserApiGroupUpdated, sendUserApiRequest as sendUserApiRequestRemote, userApiRequestCancel, onShowUserApiUpdateAlert } from '@renderer/utils/ipc'
...
export default () => {
          ...
          for (const action of actions) {
            switch (action) {
              ...
              case 'pic':
                ...
                break
              case 'mv':
                apis[source].getMvUrl = (songInfo: LX.Music.MusicInfo) => {
                  const requestKey = `request__${Math.random().toString().substring(2)}`
                  return {
                    canceleFn() {
                      userApiRequestCancel(requestKey)
  ...
  const rUserApiGroupUpdated = onUserApiGroupUpdated(({ params: { name, version } }) => {
    void dialog({
      message: t('user_api_group__updated_tip', { name, version }),
      confirmButtonText: t('ok'),
    })
    void getUserApiGroupList().then(list => {
      userApi.groupList = list
    })
