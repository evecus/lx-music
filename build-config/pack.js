...
const rendererMvConfig = './renderer-mv/webpack.config.prod'
...
function build() {
  ...
  spinners.add('renderer-mv', { text: 'renderer-mv building' })
  ...
  Promise.all([
    ...
    pack(rendererMvConfig).then(result => {
      results += result + '\n\n'
      spinners.succeed('renderer-mv', { text: 'renderer-mv build success!' })
    }).catch(err => {
      spinners.fail('renderer-mv', { text: 'renderer-mv build fail :(' })
      console.log(`\n  ${errorLog}failed to build renderer-mv process`)
      console.error(`\n${err}\n`)
      process.exit(1)
    }),
