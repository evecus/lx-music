...
const rendererMvConfig = require('./renderer-mv/webpack.config.dev')
...
function startRendererMv() {
  return new Promise((resolve, reject) => {
    const compiler = webpack(rendererMvConfig)
    hotMiddlewareRendererMv = webpackHotMiddleware(compiler, {
      log: false,
      heartbeat: 2500,
    })
    ...
    compiler.hooks.compilation.tap('compilation', compilation => {
      HtmlWebpackPlugin.getHooks(compilation).beforeEmit.tapAsync('html-webpack-plugin-after-emit', (data, cb) => {
        hotMiddlewareRendererMv.publish({ action: 'reload' })
        cb()
      })
    })
    ...
    const server = new WebpackDevServer({
      ...
      setupMiddlewares(middlewares, devServer) {
        devServer.app.use(hotMiddlewareRendererMv)
        setImmediate(() => {
          devServer.middleware.waitUntilValid(resolve)
        })
        return middlewares
...
function startMain() {
  ...
  return new Promise((resolve, reject) => {
    ...
    compiler.hooks.watchRun.tapAsync('watch-run', (compilation, done) => {
      ...
      hotMiddlewareRendererMv.publish({ action: 'compiling' })
      done()
    })
...
function init() {
  ...
  spinners.add('renderer-mv', { text: 'renderer-mv compiling' })
  ...
  Promise.all([
    ...
    startRendererMv().then(() => handleSuccess('renderer-mv')).catch((err) => {
      console.error(err.message)
      return handleFail('renderer-mv')
    }),
